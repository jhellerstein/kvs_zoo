...
//! 
//! **Dispatch:** PaxosInterceptor (total ordering via consensus)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let paxos_cfg = kvs_zoo::dispatch::PaxosConfig { f: 1, ..Default::default() };
    let dispatch = PaxosInterceptor::with_config(paxos_cfg);
