...
use kvs_zoo::{DataConsumer, DataEvent, MetaConsumer, MetaEvent, TeeAfter};
...
impl MetaConsumer<MetaEvent> for TombIndex {
    fn on_meta(&mut self, meta: &MetaEvent) {
        if let MetaEvent::Tomb { key } = meta {
            self.tombs.push(key.clone());
        }
    }
}
...
fn main() {
    ...
    let mut tee = TeeAfter::new(LogAfter, Some(TombIndex::new()));
    ...
    if let Some(bg) = &mut tee.bg_data {
        meta_consumers.push(bg as &mut dyn MetaConsumer<MetaEvent>);
    }
    for c in meta_consumers {
        c.on_meta(&tomb_meta);
    }
