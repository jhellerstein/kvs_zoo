...
impl<K, V> TimedOperation<K, V> {
    fn overlaps_with(&self, other: &Self) -> bool {
        ...
        !(self.response_time <= other.invocation_time
            || other.response_time <= self.invocation_time)
    }
}
    ...
    V: Clone + Eq + std::fmt::Debug,
{
    ...
    for &idx in order.iter() {
        ...
        match &timed_op.op {
            KVSOperation::Put(k, v, _, _) => {
                state.insert(k.clone(), v.clone());
            }
            KVSOperation::Get(k, _, _) => {
                let expected = state.get(k).cloned();
                if timed_op.response_value != expected {
                    return false;
                }
            }
            KVSOperation::Delete(k, _, _) => {
                state.remove(k);
            }
        }
    }
...
fn test_linearizable_sequential_operations() {
    ...
    let operations = vec![
        TimedOperation {
            op: KVSOperation::Put("x".to_string(), "1".to_string(), 1, Some(1)),
            invocation_time: 0,
            response_time: 10,
            response_value: None,
        },
        TimedOperation {
            op: KVSOperation::Get("x".to_string(), 2, Some(1)),
            invocation_time: 20,
            response_time: 30,
            response_value: Some("1".to_string()),
        },
        TimedOperation {
            op: KVSOperation::Put("x".to_string(), "2".to_string(), 3, Some(1)),
            invocation_time: 40,
            response_time: 50,
            response_value: None,
        },
        TimedOperation {
            op: KVSOperation::Get("x".to_string(), 4, Some(1)),
            invocation_time: 60,
            response_time: 70,
...
fn test_linearizable_concurrent_operations() {
    ...
    let operations = vec![
        ...
        TimedOperation {
            op: KVSOperation::Put("y".to_string(), "2".to_string(), 2, Some(2)),
            invocation_time: 5,
            response_time: 25,
            response_value: None,
        },
        TimedOperation {
            op: KVSOperation::Get("x".to_string(), 3, Some(1)),
            invocation_time: 30,
            response_time: 40,
            response_value: Some("1".to_string()),
        },
        TimedOperation {
            op: KVSOperation::Get("y".to_string(), 4, Some(2)),
            invocation_time: 35,
            response_time: 45,
            response_value: Some("2".to_string()),
        },
    ...
    assert!(
        is_linearizable(&operations),
        "Concurrent writes to different keys should be linearizable"
    );
}
...
fn test_linearizable_read_your_writes() {
    ...
    assert!(
        is_linearizable(&operations),
        "Read after write should see the write"
    );
}
...
fn test_non_linearizable_stale_read() {
    ...
    let operations = vec![
        ...
        TimedOperation {
            op: KVSOperation::Put("x".to_string(), "2".to_string(), 2, Some(1)),
            invocation_time: 20,
            response_time: 30,
            response_value: None,
        },
    ...
    assert!(
        !is_linearizable(&operations),
        "Stale read should violate linearizability"
    );
}
...
fn test_linearizable_overlapping_writes() {
    ...
    let operations = vec![
        ...
        TimedOperation {
            op: KVSOperation::Put("x".to_string(), "2".to_string(), 2, Some(2)),
    ...
    assert!(
        is_linearizable(&operations),
        "Overlapping writes should be linearizable"
    );
}
...
fn test_linearizable_multiple_keys() {
    ...
    let operations = vec![
        ...
        TimedOperation {
            op: KVSOperation::Put("x".to_string(), "3".to_string(), 4, Some(1)),
            invocation_time: 25,
            response_time: 35,
            response_value: None,
        },
        TimedOperation {
            op: KVSOperation::Get("x".to_string(), 5, Some(1)),
            invocation_time: 40,
            response_time: 50,
            response_value: Some("3".to_string()),
        },
        TimedOperation {
            op: KVSOperation::Get("y".to_string(), 6, Some(2)),
            invocation_time: 45,
            response_time: 55,
            response_value: Some("2".to_string()),
        },
    ...
    assert!(
        is_linearizable(&operations),
        "Multi-key operations should be linearizable"
    );
}
...
fn test_linearizable_empty_read() {
    ...
    let operations = vec![
        TimedOperation {
            op: KVSOperation::Get("x".to_string(), 1, Some(1)),
            invocation_time: 0,
            response_time: 10,
            response_value: None, // Key doesn't exist
        },
        TimedOperation {
            op: KVSOperation::Put("x".to_string(), "1".to_string(), 2, Some(1)),
            invocation_time: 20,
            response_time: 30,
            response_value: None,
        },
    ...
    assert!(
        is_linearizable(&operations),
        "Empty reads should be linearizable"
    );
}
...
fn test_paxos_provides_total_order() {
    ...
    let operations = vec![
        ...
        TimedOperation {
            op: KVSOperation::Get("x".to_string(), 3, Some(3)),
            invocation_time: 25,
            response_time: 35,
...
fn test_linearizability_with_paxos_semantics() {
    ...
    let operations = vec![
        ...
        TimedOperation {
            op: KVSOperation::Put("x".to_string(), "3".to_string(), 3, Some(3)),
            invocation_time: 10,
            response_time: 30,
            response_value: None,
        },
        ...
        TimedOperation {
            op: KVSOperation::Get("x".to_string(), 4, Some(4)),
            invocation_time: 35,
            response_time: 45,
            response_value: Some("3".to_string()),
        },
        ...
        TimedOperation {
            op: KVSOperation::Get("y".to_string(), 5, Some(5)),
            invocation_time: 36,
            response_time: 46,
            response_value: Some("2".to_string()),
        },
