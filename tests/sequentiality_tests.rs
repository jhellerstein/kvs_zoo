...
fn test_ordering_violation_detection() {
    ...
    for op in &operations {
        let response = match op {
            ...
            KVSOperation::Get(key) => {
                match sequential_state.get(key) {
                    Some(value) => format!("GET {} = {:?}", key, value),
                    None => format!("GET {} = NOT FOUND", key),
                }
            }
...
fn test_concurrent_clients_sequential_processing() {
    ...
    let operations = vec![
        // Client 1 operations
        KVSOperation::Put("shared".to_string(), LwwWrapper::new("client1_v1".to_string())),
        // Client 2 operations  
        KVSOperation::Put("shared".to_string(), LwwWrapper::new("client2_v1".to_string())),
        // Client 1 reads
        KVSOperation::Get("shared".to_string()),
        // Client 2 reads
        KVSOperation::Get("shared".to_string()),
        // Client 1 updates
        KVSOperation::Put("shared".to_string(), LwwWrapper::new("client1_v2".to_string())),
        // Both clients read
        KVSOperation::Get("shared".to_string()),
        KVSOperation::Get("shared".to_string()),
    ];
