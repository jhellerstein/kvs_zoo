...
fn test_linearizable_kvs_concurrent_clients() {
    ...
    let operations = vec![
        ...
        KVSOperation::Put("shared_counter".to_string(), LwwWrapper::new("0".to_string())),
        // Client B reads
        KVSOperation::Get("shared_counter".to_string()),
        // Client A increments
        KVSOperation::Put("shared_counter".to_string(), LwwWrapper::new("1".to_string())),
        // Client B reads again
        KVSOperation::Get("shared_counter".to_string()),
        // Client B increments
        KVSOperation::Put("shared_counter".to_string(), LwwWrapper::new("2".to_string())),
        // Client A reads
        KVSOperation::Get("shared_counter".to_string()),
        // Client A increments
        KVSOperation::Put("shared_counter".to_string(), LwwWrapper::new("3".to_string())),
...
fn test_linearizable_kvs_replica_consistency() {
    ...
    println!("Replica states are identical: {}", replica1_state == replica2_state);
}
