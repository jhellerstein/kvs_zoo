//! Metadata channel types and handlers for the After pipeline.
//!
//! This module defines metadata messages (`MetaMessage`) that can be replicated and handled
//! independently from data replication. Metadata messages enable operations such as
//! tombstone reclamation, compaction hints, or other auxiliary signals without routing through the
//! main `KVSOperation` stream.
//!
//! # Metadata Message Flow
//!
//! Metadata messages follow a separate path from data operations:
//!
//! ```text
//! KVSOperation Stream
//!        ↓
//!    [Split]
//!    ↙      ↘
//! Data      Metadata (MetaMessage)
//!  ↓         ↓
//!  ↓         ↓
//!  ↓      MetaLocalHandler (optional)
//!  ↓         ↓
//!  └────┬────┘
//!       ↓
...
//!
//! ```ignore
//! use kvs_zoo::after_storage::meta::{MetaMessage, TombMeta, ReclaimMeta};
//!
//! // Create a tombstone for a deleted key
//! let tomb = MetaMessage::tomb("user:alice".to_string());
//!
//! // Create a reclamation frontier
//! let reclaim = MetaMessage::reclaim_with_frontier(1000, 1);
//!
//! // Extract metadata messages from operations
