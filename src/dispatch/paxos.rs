...
impl<V> PaxosDispatcher<V> {
    /// Create a new Paxos dispatcher with default configuration
    pub fn new() -> Self {
        Self {
            config: PaxosConfig {
    ...
    /// Create a new Paxos dispatcher with custom configuration
    ...
    /// For true global slot numbers, use `dispatch_operations_slotted_with_paxos` with properly
    /// deployed Proposer and Acceptor clusters.
    ///
    /// TODO: Integrate real Paxos by solving the cluster deployment challenge
    pub fn dispatch_operations_slotted_simple<'a>(
        &self,
    ...
    #[allow(dead_code)]
    pub fn dispatch_operations_slotted_with_paxos<'a>(
        &self,
        operations: Stream<KVSOperation<V>, Process<'a, ()>, Unbounded>,
        proxy: &Process<'a, ()>,
        proposers: &Cluster<'a, Proposer>,
...
mod tests {
    ...
    fn test_paxos_dispatcher_creation() {
        let paxos = PaxosDispatcher::<String>::new();
